...
const DockerSource = "DockerSwarm"
...
func (tg *targetGroup) sync(groups []*targetgroup.Group) {
	...
	for _, group := range groups {
		...
		for _, t := range group.Targets {
			taskID, ok := t[dockerLabelTaskID]
			if !ok {
				level.Debug(tg.logger).Log("msg", "Docker Swarm target did not include task ID")
				continue
			}
			err := tg.addTarget(string(taskID), t)
			if err != nil {
				level.Error(tg.logger).Log("msg", "could not add target", "taskID", taskID, "err", err)
			}
		}
	}
}
...
func (tg *targetGroup) addTarget(id string, discoveredLabels model.LabelSet) error {
	if tg.client == nil {
		...
		if hostURL.Scheme == "http" || hostURL.Scheme == "https" {
			rt, err := config.NewRoundTripperFromConfig(tg.httpClientConfig, "dockerswarm_sd")
			if err != nil {
				return err
			}
			opts = append(opts,
	...
	if t, ok := tg.targets[id]; ok {
		level.Debug(tg.logger).Log("msg", "container target already exists", "task", id)
		t.startIfNotRunning()
		return nil
	}
	...
	t, err := NewTarget(
		tg.metrics,
		log.With(tg.logger, "target", fmt.Sprintf("dockerswarm/%s", id)),
		tg.entryHandler,
		tg.positions,
		id,
	...
	tg.targets[id] = t
	level.Info(tg.logger).Log("msg", "added Docker Swarm target", "taskID", id)
	return nil
}
