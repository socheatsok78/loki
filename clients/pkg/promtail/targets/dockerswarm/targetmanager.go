...
const (
	...
	dockerLabel                = model.MetaLabelPrefix + "dockerswarm_"
	...
	dockerLabelServiceID       = dockerLabel + "service_id"
	dockerLabelTaskID          = dockerLabel + "task_id"
)
...
func NewTargetManager(
	...
	tm := &TargetManager{
		...
		positions:  positions,
		manager:    discovery.NewManager(ctx, log.With(logger, "component", "dockerswarm_discovery")),
		pushClient: pushClient,
		groups:     make(map[string]*targetGroup),
	}
	configs := map[string]discovery.Configs{}
	for _, cfg := range scrapeConfigs {
		if cfg.DockerSwarmSDConfigs != nil {
			pipeline, err := stages.NewPipeline(
				log.With(logger, "component", "dockerswarm_pipeline"),
				cfg.PipelineStages,
				&cfg.JobName,
				metrics.reg,
			)
			...
			for _, sdConfig := range cfg.DockerSwarmSDConfigs {
			...
			level.Debug(tm.logger).Log("msg", "Docker Swarm service discovery configs are empty")
		}
	}
